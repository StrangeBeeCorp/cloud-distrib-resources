...
# We must locate some existing resources to set the run context for the Cortex instance
...
# We look for the Cortex security group based on its name so that we can refer to its ID later on
data "aws_security_group" "secops-sg-cortex" {
  filter {
    name   = "tag:Name"
    values = [var.secops_cortex_sg_name]
  }
}
...
# We look for the latest Cortex AMI based on search filters so that we can refer to its ID later on
...
# We look for the Cortex ALB target group based on its name so that we can refer to its ID later on
data "aws_lb_target_group" "secops-alb-tg-cortex" {
  name = var.secops_alb_target_group_name_cortex
}
