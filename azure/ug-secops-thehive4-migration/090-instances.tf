...
resource "azurerm_virtual_machine" "secops-thehive-instance" {
  ...
  os_profile {
    ...
    custom_data = file("files/th-cloud-config-restore.yaml")
  }
  ...
  storage_data_disk {
    ...
    managed_disk_id = data.azurerm_managed_disk.thehive-data-disk.id
    disk_size_gb  = data.azurerm_managed_disk.thehive-data-disk.disk_size_gb    
    lun = 0
  ...
  storage_data_disk {
    ...
    caching = "ReadWrite"
    create_option = "Attach"
    managed_disk_id = data.azurerm_managed_disk.thehive-storage-disk.id
    disk_size_gb  = data.azurerm_managed_disk.thehive-storage-disk.disk_size_gb    
    lun = 1
    managed_disk_type = var.secops-data-disk-type
    }
  storage_data_disk {
    ...
    caching = "ReadWrite"
    create_option = "Attach"
    managed_disk_id = data.azurerm_managed_disk.thehive-index-disk.id
    disk_size_gb  = data.azurerm_managed_disk.thehive-index-disk.disk_size_gb    
    lun = 2
    managed_disk_type = var.secops-data-disk-type
    }
