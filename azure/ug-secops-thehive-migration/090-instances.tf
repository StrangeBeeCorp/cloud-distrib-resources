...
# az vm image terms show --publisher strangebee1595948424730 --plan thehive3 --offer thehive-3
resource "azurerm_virtual_machine" "secops-thehive-instance" {
  ...
  storage_data_disk {
    ...
    managed_disk_id = data.azurerm_managed_disk.thehive-es-data-disk.id
    disk_size_gb  = data.azurerm_managed_disk.thehive-es-data-disk.disk_size_gb    
    lun = 0
