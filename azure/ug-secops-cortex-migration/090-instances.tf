...
# You must accept the marketplace terms using the following command before launching your first Cortex instance:
# az vm image terms accept --urn strangebee1595948424730:cortex-3:cortex3:3.1.0
# To check terms status :
# az vm image terms show --publisher strangebee1595948424730 --plan cortex3 --offer cortex-3
resource "azurerm_virtual_machine" "secops-cortex-instance" {
  ...
  os_profile {
    ...
    custom_data = file("files/cortex-cloud-config-restore.yaml")
  }
  ...
  storage_data_disk {
    ...
    managed_disk_id = data.azurerm_managed_disk.cortex-es-data-disk.id
    disk_size_gb  = data.azurerm_managed_disk.cortex-es-data-disk.disk_size_gb    
    lun = 0
  ...
  storage_data_disk {
    ...
    caching = "ReadWrite"
    create_option = "Attach"
    managed_disk_id = data.azurerm_managed_disk.cortex-docker-data-disk.id
    disk_size_gb  = data.azurerm_managed_disk.cortex-docker-data-disk.disk_size_gb    
    lun = 1
    managed_disk_type = var.secops-data-disk-type
    }
