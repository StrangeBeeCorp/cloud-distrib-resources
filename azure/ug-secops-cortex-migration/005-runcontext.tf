...
data "azurerm_managed_disk" "cortex-es-data-disk" {
  name                = var.secops-data-disk-name
  resource_group_name = var.secops-resource-group-name
}
# cortex-es-data-disk-id = "${azurerm_managed_disk.cortex-es-data-disk.id}"

data "azurerm_managed_disk" "cortex-docker-data-disk" {
  name                = var.secops-docker-disk-name
  resource_group_name = var.secops-resource-group-name
}
# cortex-docker-data-disk-id = "${azurerm_managed_disk.cortex-docker-data-disk.id}"
